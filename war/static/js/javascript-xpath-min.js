...
@*/
if(type==9||type==1){if(type==9){node=node.documentElement}else{node=node.firstChild}for(t="",stack=[],i=0;node;){do{if(node.nodeType!=1){t+=node.nodeValue;
/*@cc_on
                        else if (node.nodeName.toLowerCase() == 'title') {
                            t += node.text;
                        }
@*/
}stack[i++]=node}while(node=node.firstChild);while(i&&!(node=stack[--i].nextSibling)){}}}else{t=node.nodeValue}}switch(valueType){case"number":return +t;case"boolean":return !!t;default:return t}},attrPropMap:{name:"name","class":"className",dir:"dir",id:"id",name:"name",title:"title"},attrMatch:function(node,attrName,attrValue){
/*@cc_on @if (@_jscript)
...
/*@end @*/
return true}else{return false}},getDescendantNodes:function(test,node,nodeset,attrName,attrValue,prevNodeset,prevIndex){if(prevNodeset){prevNodeset.delDescendant(node,prevIndex);
/*@cc_on
        try {
...
@*/
for(var node=node.firstChild;node;node=node.nextSibling){if(NodeUtil.attrMatch(node,attrName,attrValue)){if(test.match(node)){nodeset.push(node)}}}return nodeset;
/*@cc_on
        }
@*/
}};
...
nodeset.push(attr);
/*@end @*/
}}else{var attr=attrs.getNamedItem(test.name);
/*@cc_on @if (@_jscript)
...
@else @*/
var NodeID={uuid:1,get:function(node){return node.__jsxpath_id__||(node.__jsxpath_id__=this.uuid++)}};
/*@end @*/
if(!window.NodeSet&&window.defaultConfig){window.NodeSet=null}NodeSet=function(){this.length=0;this.nodes=[];this.seen={};this.idIndexMap=null;this.reserveDels=[]};NodeSet.prototype.isNodeSet=true;NodeSet.prototype.isSorted=true;NodeSet.prototype.merge=function(nodeset){this.isSorted=false;if(nodeset.only){return this.push(nodeset.only)}if(this.only){var only=this.only;delete this.only;this.push(only);this.length--}var nodes=nodeset.nodes;for(var i=0,l=nodes.length;i<l;i++){this._add(nodes[i])}};NodeSet.prototype.sort=function(){if(this.only){return}if(this.sortOff){return}if(!this.isSorted){this.isSorted=true;this.idIndexMap=null;
/*@cc_on
        if (this.shortcut) {
            this.nodes.sort();
        }
        ...
        return;
@*/
var nodes=this.nodes;nodes.sort(function(a,b){if(a==b){return 0}if(a.compareDocumentPosition){var result=a.compareDocumentPosition(b);if(result&2){return 1}if(result&4){return -1}return 0}else{var node1=a,node2=b,ancestor1=a,ancestor2=b,deep1=0,deep2=0;while(ancestor1=ancestor1.parentNode){deep1++}while(ancestor2=ancestor2.parentNode){deep2++}if(deep1>deep2){while(deep1--!=deep2){node1=node1.parentNode}if(node1==node2){return 1}}else{if(deep2>deep1){while(deep2--!=deep1){node2=node2.parentNode}if(node1==node2){return -1}}}while((ancestor1=node1.parentNode)!=(ancestor2=node2.parentNode)){node1=ancestor1;node2=ancestor2}while(node1=node1.nextSibling){if(node1==node2){return -1}}return 1}})}};
/*@cc_on @if (@_jscript)
...
@else @*/
NodeSet.prototype.reserveDelByNodeID=function(id,offset,reverse){var map=this.createIdIndexMap();var index;if(index=map[id]){if(reverse&&(this.length-offset-1)>index||!reverse&&offset<index){var obj={value:index,order:String.fromCharCode(index),toString:function(){return this.order},valueOf:function(){return this.value}};this.reserveDels.push(obj)}}};
/*@end @*/
...
/*@end @*/
};NodeSet.prototype.doDel=function(){if(!this.reserveDels.length){return}if(this.length<65536){var dels=this.reserveDels.sort(function(a,b){return b-a})}else{var dels=this.reserveDels.sort(function(a,b){return b-a})}for(var i=0,l=dels.length;i<l;i++){this.del(dels[i])}this.reserveDels=[];this.idIndexMap=null};NodeSet.prototype.createIdIndexMap=function(){if(this.idIndexMap){return this.idIndexMap}else{var map=this.idIndexMap={};var nodes=this.nodes;for(var i=0,l=nodes.length;i<l;i++){var node=nodes[i];
/*@cc_on @if (@_jscript)
            var sourceIndex = node.sourceIndex;
...
@else @*/
var id=NodeID.get(node);map[id]=i;
/*@end @*/
}return map}};NodeSet.prototype.del=function(index){this.length--;if(this.only){delete this.only}else{var node=this.nodes.splice(index,1)[0];if(this._first==node){delete this._first;delete this._firstSourceIndex;delete this._firstSubIndex;
/*@cc_on @if (@_jscript)
        delete this.seen[node.sourceIndex][node.subIndex];
@else @*/
}delete this.seen[NodeID.get(node)];
/*@end @*/
}};NodeSet.prototype.delDescendant=function(elm,offset){if(this.only){return}var nodeType=elm.nodeType;if(nodeType!=1&&nodeType!=9){return}if(uai.applewebkit2){return}if(!elm.contains){if(nodeType==1){var _elm=elm;elm={contains:function(node){return node.compareDocumentPosition(_elm)&8}}}else{elm={contains:function(){return true}}}}var nodes=this.nodes;for(var i=offset+1;i<nodes.length;i++){
/*@cc_on @if (@_jscript)
        if (nodes[i].node.nodeType == 1 && elm.contains(nodes[i].node)) {
@else @*/
if(elm.contains(nodes[i])){
/*@end @*/
this.del(i);i--}}};NodeSet.prototype._add=function(node,reverse){
/*@cc_on @if (@_jscript)
...
@else @*/
var seen=this.seen;var id=NodeID.get(node);if(seen[id]){return true}seen[id]=true;
/*@end @*/
this.length++;if(reverse){this.nodes.unshift(node)}else{this.nodes.push(node)}};NodeSet.prototype.unshift=function(node){if(!this.length){this.length++;this.only=node;return}if(this.only){var only=this.only;delete this.only;this.unshift(only);this.length--;
/*@cc_on
    node = this.createWrapper(node);
@*/
}return this._add(node,true)};NodeSet.prototype.push=function(node){if(!this.length){this.length++;this.only=node;return}if(this.only){var only=this.only;delete this.only;this.push(only);this.length--;
/*@cc_on
    node = this.createWrapper(node);
@*/
}return this._add(node)};NodeSet.prototype.first=function(){if(this.only){return this.only;
/*@cc_on
...
@*/
}if(this.nodes.length>1){this.sort()}return this.nodes[0]};NodeSet.prototype.list=function(){if(this.only){return[this.only]}this.sort();
/*@cc_on
    ...
    return results;
@*/
return this.nodes};NodeSet.prototype.string=function(){var node=this.only||this.first();return node?NodeUtil.to("string",node):""};NodeSet.prototype.bool=function(){return !!(this.length||this.only)};NodeSet.prototype.number=function(){return +this.string()};NodeSet.prototype.iterator=function(reverse){this.sort();var nodeset=this;if(!reverse){var count=0;return function(){if(nodeset.only&&count++==0){return nodeset.only;
/*@cc_on @if(@_jscript)
...
/*@end @*/
}}else{var count=0;return function(){var index=nodeset.length-(count++)-1;if(nodeset.only&&index==0){return nodeset.only;
/*@cc_on @if(@_jscript)
